    ...
    public Revenue_Report() {
     ...
     private void loadlabel(){
        try {
            ...
            ResultSet rs1 = MYSQL.execute("SELECT SUM(paid_amount) AS total FROM `order_payments`WHERE `order_date`='"+currentDate+"'");
            ResultSet rs2 = MYSQL.execute("SELECT COUNT(reservation_customer_id) AS row_count  FROM `reservation_customer`");
            
               if (rs1.next()) {
                double totalrevenue = rs1.getDouble("total");
                jLabel6.setText(String.valueOf(totalrevenue));
                }
               
               if(rs2.next()){
                   int rowCount = rs2.getInt("row_count");
